...
- [x] 復号エラー（`AuthenticationFailed`（パスフレーズ不正・machine_id 不一致・改ざん等を含む）、ファイル未存在など）はすべて **stderr への出力なし**、終了コード **1** で即終了
...
| vault を machine_id が異なるホストにコピー | Locked Mode で Argon2id が machine_id にバインド |
