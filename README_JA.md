...
Amulet は、秘密情報（API キー・トークン・パスワード等）を **特定の物理マシン** に暗号化バインドする CLI ツールです。
...
次を順に確認してください: パスフレーズ（`seal` 時と同じか）→ vault のパス（`--file` が正しいか）→ キー名（大文字・小文字含め正確か、`amulet list` で確認）→ Locked モード（このマシンで seal したか）。
...
|--------|-------------|----------------|
| **Locked**（デフォルト） | パスフレーズ + マシンのハードウェア ID | seal したマシンのみ |
