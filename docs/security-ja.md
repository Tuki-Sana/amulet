...
**安定性:** 再起動には耐えますが、OS の再インストール（Linux）やマザーボード交換（macOS）で変わります。

### Portable Mode（`--portable` 付きで seal した場合）

machine_id を KDF に混ぜません。パスフレーズだけで、どのマシンでも復号できます。
...
| vault を別マシンにコピーされる | Locked Mode で Argon2id が machine_id にバインド |
