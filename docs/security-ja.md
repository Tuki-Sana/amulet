...
`N` は直後の ciphertext のバイト数だけを表し、Poly1305 の認証タグ（16 バイト）は含みません。ChaCha20-Poly1305 では ciphertext の長さは平文長と一致するため、`N` は seal 時の秘密のバイト数（実装では `plaintext.len` を書き込む）と同じになります。
